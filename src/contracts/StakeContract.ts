...
export type StakeContractConfig = {
  // {TODO}
  address: Address;
  owner_address: Address;
};

export function MainContractConfigToCell(config: StakeContractConfig): Cell {
  return beginCell()
    // {TODO}
...
export class StakeContract implements Contract {
  ...
  static async createFromConfig(config: StakeContractConfig, code: Cell, workchain = 0) {
    const data = MainContractConfigToCell(config);
    const init = { code, data };
    const address = contractAddress(workchain, init);

    return new StakeContract(address, init);
  }
